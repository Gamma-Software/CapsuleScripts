...
if [ $? -eq 0 ]; then
    echo "Le blog est déjà en cours d'édition et disponible à l'adresse suivante http://192.168.10.241:5000"
else
    ...
    echo -e "Le blog est lancé et disponible dans quelques minutes à l'adresse suivante sur http://192.168.10.241:5000"
    echo -e "le blog n'est disponible que dans le reseau local et n'est pas partagé au monde entier."
fi

docker ps | grep blog-editor
if [ $? -eq 0 ]; then
    echo "L'editeur du blog est déjà en cours d'édition et disponible à l'adresse suivante http://192.168.10.241:9888"
else
    ...
    echo -e "Son edition est possible sur http://192.168.10.241:9888"
