...
print(datetime.datetime.now(), ": ", "network_scan_info: ", network_scan_info.stdout.splitlines())
...
print(datetime.datetime.now(), ": ", "known_networks_info:", known_networks_info.stdout.splitlines())
...
print(datetime.datetime.now(), ": ", "active_connection:", active_connection.split(":")[1])
    ...
    if int(strongest_signal) > int(current_signal_strength)+MIN_SIGNAL_STRENGTH_DIFFERENCE_FOR_SWITCHING:
        subprocess.run(
             ["/usr/bin/nmcli",
                    ...
                    "connect",
                    strongest_name])
        print(datetime.datetime.now(), ": ", "switch to:", strongest_name)